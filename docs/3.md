...
> Why all the blank values? Well, when the Vue instance is created, it adds all the properties found in its data object to Vue's reactivity system. All the values above are going to change as we render our cards, so we need to create them initially in our data object so their value can be tracked.
